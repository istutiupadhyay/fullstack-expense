...
window.addEventListener('DOMContentLoaded',()=>{
    axios.get('http://localhost:3000/user/getallexpenses',{headers:{'Authorization':token}})
    .then(data=>{
...
function showExpenseOnScreen(data){
    ...
    const datas=`<li id=${data.id}> ${data.expenseamount}------ ${data.description}------ ${data.category} </li>`
...
function showDownloadedHistoryOnScreen(data){
    ...
    const downloads=`<li>${data.url}-------------------------${data.createdAt}</li>`
